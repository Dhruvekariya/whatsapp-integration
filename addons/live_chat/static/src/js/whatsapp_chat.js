...
class WhatsAppChat extends Component {
    ...
    async selectChat(chat) {
        ...
        if (!this.state.messages[chat.id]) {
            try {
                const messages = await this.rpc("/live_chat/whatsapp/messages/" + chat.id);
                this.state.messages[chat.id] = messages.map(msg => {
    ...
    async sendMessage() {
        ...
        try {
            ...
            const result = await this.rpc("/live_chat/whatsapp/send", {
                chat_id: this.state.activeChat.id,
                message: messageText
            });
            ...
            if (result.success) {
                ...
                if (messageIndex >= 0) {
                    this.state.messages[this.state.activeChat.id][messageIndex] = {
                        ...result.message,
                        time: new Date(result.message.time).toLocaleTimeString([], {
                            hour: '2-digit',
                            minute: '2-digit'
                        }),
                        sending: false,
...
WhatsAppChat.template = "whatsapp_integration_test.WhatsAppChat";

// Register the dashboard action
registry.category("actions").add("whatsapp_integration_test.dashboard", WhatsAppChat);